    ...
    def __init__(self) -> None:
        ...
        if settings.storage_backend == "s3":
            if not all(
                ...
                raise RuntimeError("S3 storage is enabled but required S3 settings are missing")
            self.provider = SpacesStorageProvider(
                bucket=settings.s3_bucket,
                region=settings.s3_region,
                endpoint_url=settings.s3_endpoint_url or f"https://{settings.s3_region}.digitaloceanspaces.com",
